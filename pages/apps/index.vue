<template>
    <div class="w-full flex flex-col md:flex-row flex-wrap items-stretch justify-center py-4 bg-gray-50" style="z-index:20;">
        <div class="px-0 md:px-6 lg:px-10 pb-10 flex flex-wrap relative items-start justify-center" style="z-index:20;">
            <div class="w-full h-16 flex flex-wrap items-start justify-center mb-4 px-4 relative" style="z-index:20;">
                <span style="z-index:20;" class="font-bold text-4xl text-gray-100 relative" v-scroll-reveal="{ delay: 100, mobile: true, origin: 'top', easing: 'cubic-bezier(0.5, 0, 0, 1)', distance: '80px'}">Apps</span>
            </div>
            <div class="w-full flex flex-wrap items-center justify-center relative">
                <div class="w-auto pb-8 block md:inline-block relative" style="z-index:20;">
                    <ul class="list-style-1 relative" style="z-index:20;">
            ...
            <div class="w-full max-w-6xl flex flex-wrap -mx-4 justify-center items-stretch">
                <div v-for="(app, i) in apps" :key="i" class="my-4 px-4 w-full md:w-1/2 lg:w-1/3 flex flex-wrap justify-between items-stretch select-none">
                    <div class="w-full rounded-lg duration-300 transition-all grid grid-rows-2 shadow-xl bg-gray-100">
                        ...
                        <div class="px-8 pb-6 rounded-b-md md:rounded-b-lg w-full flex flex-wrap content-between bg-white">
                            ...
                            <div class="w-full flex flex-wrap">
                                <nuxt-link :to="`${app.path}/`" class="text-2xl font-medium text-gray-800">{{app.title}}</nuxt-link>
                                <p class="text-base text-gray-600 leading-tight my-2 font-light" style="-webkit-box-orient: vertical;-webkit-line-clamp: 2;line-height:1.5rem; max-height: 3rem;overflow: hidden;white-space: normal;text-overflow: ellipsis;-o-text-overflow: ellipsis;">{{app.description | desc}}</p>
                                <div class="mt-4 w-full flex flex-wrap justify-between items-stretch self-stretch">
                                    <div class="flex flex-wrap items-center justify-center gap-x-3" >
                                        ...
                                        <span class="text-base font-secondary text-blue-700 ml-2">{{app.author}}</span>
                                    </div>
                                    <nuxt-link :to="`${app.path}/`" class="btn-secondary focus:outline-none focus:ring focus:ring-blue-400">
                                        Read More
...
.list-style-1 {
    @apply text-gray-100 w-full flex flex-wrap flex-col justify-center py-4 px-8 rounded-lg shadow-2xl;
...
.bg-apps{
    background-image: url('/assets/bg-apps.svg');
...
.btn-view-more{
    @apply bg-transparent text-blue-500 p-1 rounded-md text-base font-secondary flex flex-wrap justify-center items-center text-center transition-all duration-300;
    transition: all 0.4s;
...
.btn-secondary{
    @apply bg-transparent text-gray-700 py-1 leading-none rounded-full text-base font-secondary flex flex-wrap justify-center items-center text-center border-2 border-transparent transition-all duration-300 ease-in-out;
    & > svg {
...
.card-img{
    ...
    @apply bg-gradient-to-br from-green-300 to-blue-500 flex flex-wrap items-stretch justify-center;
